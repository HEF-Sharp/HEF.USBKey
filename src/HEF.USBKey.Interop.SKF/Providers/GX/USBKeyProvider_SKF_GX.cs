...
{
    public class USBKeyProvider_SKF_GX : USBKeyProvider_SKF_Base, IUSBKeyProvider_SKF
    {
        public const string SKF_ProviderName = "GX";
        ...
        {
            return Native_SKF_GX.SKF_ChangePIN(hApplication, ulPINType, szOldPin, szNewPin, out pulRetryCount);
        }

        protected override uint Native_SKF_VerifyPIN(IntPtr hApplication, uint ulPINType, string szPin, out uint pulRetryCount)
        {
            return Native_SKF_GX.SKF_VerifyPIN(hApplication, ulPINType, szPin, out pulRetryCount);
        }
        #endregion
    }
}
